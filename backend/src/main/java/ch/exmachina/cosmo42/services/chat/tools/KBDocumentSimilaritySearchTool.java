...
public class KBDocumentSimilaritySearchTool extends BaseTool {
	...
	public KBSimilaritySearchResponse search(
			@ToolParam(description = "A specific, context-rich search query written in natural language, designed to retrieve the most semantically relevant documents.", required = true) String query,
			ToolContext context) {
		emitStatus(context, "Searching Knowledge Base...");

		EmbeddingResponse embeddingResponse = embeddingModel
				.call(new EmbeddingRequest(List.of(query), embeddingModelOptions));
		...
		var chunks = kbDocumentChunkRepository.findMostSimilarByCosine(bytesVector, 0.5, 10);
